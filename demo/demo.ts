...
document.addEventListener("DOMContentLoaded", () => {
	...
	const checkInterval = setInterval(() => {
		if (window.thumbnailGen !== undefined && window.thumbnailGen.isInitialized()) {
...
// Handle file input changes. The listener is wrapped rather than being async
// itself: addEventListener discards the returned promise, so a rejection inside
// an async handler surfaces as an unhandled rejection instead of a caught error.
